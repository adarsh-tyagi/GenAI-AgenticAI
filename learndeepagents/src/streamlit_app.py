...
basicsdeepagent.ipynb   -> create_deep_agent, custom model, custom system
                             prompt, custom tools (Tavily web search),
                             built-in planning (write_todos) + virtual files
contextengineering.ipynb -> AGENTS.md context file, memory=, checkpointer +
                             thread_id conversation memory, Skills (/skills/)
backends.ipynb          -> StateBackend / FilesystemBackend / StoreBackend
subagents.ipynb         -> custom subagents (research-agent) + structured
...
# Environment (load API keys from .env)
ROOT_DIR = Path(__file__).parent.parent
DEMO_DIR = ROOT_DIR / "src"
...
# Structured output schema (structured output with subagents)
...
# Context engineering helpers (AGENTS.md + skills seeding)
...
def build_agent(cfg: dict):
    ...
    kwargs = dict(
        ...
        backend=backend,
        checkpointer=st.session_state.checkpointer,  # thread memory
    )
    if subagents:
        kwargs["subagents"] = subagents
    if cfg["use_skills"]:
        kwargs["skills"] = ["/skills/"]  # Agent Skills
    if memory_paths:
        kwargs["memory"] = memory_paths  # memory= context loading
...
with st.sidebar:
    ...
    model = st.selectbox(
        ...
        index=0,
        help="customizing the deep agent's model "
    ...
    backend = st.radio(
        ...
        help="StateBackend = ephemeral per-thread; "
    ...
    use_agents_md = st.checkbox(
        ...
        help="durable 'who you are' context from projects/AGENTS.md",
    )
    use_skills = st.checkbox(
        "Skills (/skills/)", value=True,
        help="langgraph, python, aws, report-writer skills",
    )
    use_subagents = st.checkbox(
        "Subagents", value=True,
        help="research-agent + structured-output researcher",
    )
    if use_subagents:
    ...
    system_prompt = st.text_area(
        ...
        help="custom system prompt layered on the built-in "
